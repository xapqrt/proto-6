...
import TransitionOverlay from './components/auth/transition-overlay'; // Updated path
import TransitionStyles from './components/auth/transition-styles'; // Updated path
import CriticalAuthGuard from './components/auth/critical-auth-guard'; // Updated path
import { startGlobalTransition, endGlobalTransition } from './lib/auth-transition-controller'; // Updated path
...
import { AuthDebugger } from './components/auth/debug-utils'; // Updated path
...
function AppContent() {
  ...
  const showSidebar = user && !authLoading && !isTransitioning && authState === 'authenticated';
    ...
    <div className={cn('flex min-h-screen w-full', 'antialiased font-sans')}>
      ...
      >
        <Routes>
          ...
          {(!user || isTransitioning || authState !== 'authenticated') ? (
              ...
              <Route path="*" element={
                <div className="flex flex-col justify-center items-center h-screen w-screen bg-background">
                  ...
                  <span className="mt-4 text-muted-foreground">
                    {isTransitioning ? "Transitioning to dashboard..." : 
