...
export default function LoginPage() {
  ...
  const { signIn, loading: authHookLoading, user } = useAuth(); // Renamed loading to authHookLoading to avoid conflict
  const { toast } = useToast();
  const navigate = useNavigate();
  const location = useLocation();

  useEffect(() => {
    // This effect handles redirection if user is already logged in when visiting /login
    if (user && !authHookLoading) { // Use authHookLoading
      if (location.pathname === '/login') {
        console.log('LoginPage: User already authenticated, redirecting to /dashboard');
        navigate('/dashboard', { replace: true });
      }
    }
  }, [user, authHookLoading, navigate, location.pathname]);

  useEffect(() => {
    // Prefetch routes when component mounts and user is not logged in
    if (!user && !authHookLoading) {
      APP_ROUTES.forEach(route => {
        // Prefetching logic for React Router
      });
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    // signIn function from useAuth already handles redirect on success
    await signIn(email, password);
    // No explicit navigate here, as useAuth's handleApiAuth will do it.
  };

   // Show a loader specifically for the auth operation itself (signIn)
   // The AuthProvider handles the broader initial loading state.
   if (authHookLoading && !user && (location.pathname === '/login' || location.pathname === '/signup')) {
     return (
       <div className="flex justify-center items-center h-screen w-screen bg-background">
         <Loader2 className="w-12 h-12 animate-spin text-primary" />
         <span className="ml-3 text-muted-foreground">Logging In...</span>
       </div>
     );
   }

  // If user becomes available and we are still on login, the useEffect above will redirect
  // So, if we reach here and `user` is set, it's likely a transient state before redirect.
  // It's generally safe to render the form, as the redirect will happen quickly.
