...
import { RecentActivityList, type ActivityItem as BaseActivityItem } from "@/components/dashboard/recent-activity-list";
...
// Extended ActivityItem interface that includes timestamp
...
const AdminDataView = () => {
    ...
    }, [isAdmin, getAllUsers, toast, adminDataLoading]);

    useEffect(() => {
        // Only fetch data when necessary - prevent infinite loops
        if (isAdmin && !authLoading && !hasInitialFetch && !adminDataLoading) {
            console.log('AdminDataView: Initial data fetch');
            fetchAdminData();
        }
    }, [isAdmin, authLoading, fetchAdminData, hasInitialFetch, adminDataLoading]);
...
export default function DashboardPage() {
    ...
    dateFields.forEach(field => {
        if (newItem[field] && typeof newItem[field] === 'string') {
            ...
            if (isValid(date)) {
                ...
                newItem[field] = date as T[typeof field];
            } else {
                console.warn(`Invalid date string found for field ${String(field)} in item:`, item);
                // Use type assertion to fix the TypeScript error
                newItem[field] = new Date() as T[typeof field];
            }
  ...
  const processedData = useMemo(() => {
    ...
    const combinedActivities = [...taskActivities, ...sessionActivities]
        .sort((a, b) => {
            // Safely handle optional timestamp property
